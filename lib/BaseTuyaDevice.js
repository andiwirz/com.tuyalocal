...
const { leseWerteliste, hatZuordnung, rohZuId } = require('./utils.js');

const SNAPSHOT_DEBOUNCE_MS  = 3000;   // persist dp_snapshot max once per 3 s
// Obergrenze fuer den gesamten dp_snapshot, in Zeichen des JSON.
//
// Gemeldet von Michel Helsdingen aus seiner eigenen Tuya-App: sein Geraeteregister trug
// die vollstaendige DP-Spezifikation mit, 88 % eines 110-KB-Blobs, und war damit zu gross
// fuer Homeys App-Einstellungen. Das Schreiben scheiterte wortlos, nach einem Neustart
// war das Register leer und jedes Geraet sah tot aus.
//
// Uns kann das nicht so weh tun - dp_snapshot ist Diagnose, kein Register, und Homey
// verwaltet unsere Geraete selbst. Aber die Form ist dieselbe: alle Live-DPs samt rawMeta
// jedes Geraets in einem Schluessel, ohne Grenze. 64 KB liegt deutlich unter dem, woran
// es bei ihm scheiterte, und reicht fuer weit mehr Geraete, als jemand betreibt.
const SNAPSHOT_MAX_CHARS   = 64 * 1024;
// Wie viele Untergeraete eines Gateways gemerkt werden. Ein Tuya-Gateway traegt in der
// Praxis selten mehr als ein paar Dutzend; die Grenze ist da, damit ein gespraechiges
// nicht den ganzen Schnappschuss fuellt.
const MAX_CIDS             = 40;
...
class BaseTuyaDevice extends Homey.Device {
  ...
  async _baseInit() {
    ...
    // Untergeraete hinter einem Gateway. Ein Tuya-Gateway schickt die Werte seiner
    // Zigbee- und Bluetooth-Geraete ueber dieselbe Verbindung wie seine eigenen; das
    // einzige, was sie unterscheidet, ist ein Feld cid im Rohpaket. Diese Kennung
    // stand bisher nur im letzten Paket und war nirgends ablesbar — ohne sie laesst
    // sich ein Untergeraet weder benennen noch ansprechen.
    this._cidSeen               = {};    // cid -> { dps, pakete, zuerst, zuletzt }
    ...
    // Die Untergeraete eines Gateways zurueckholen, statt sie beim Neustart zu
    // vergessen. Sie standen bisher nur im Arbeitsspeicher: nach jedem App-Start war
    // die Liste leer, bis sich jedes Untergeraet von selbst wieder meldete — und ein
    // Heizkoerperthermostat schweigt auch mal eine halbe Stunde. Ein Meldender sah
    // sein Bluetooth-Geraet nach einer Aktualisierung verschwinden und schloss
    // daraus, es sei nicht mehr erreichbar. Genau der falsche Schluss, und ich hatte
    // ihm selbst gesagt, dass eine leere Liste diese Bedeutung habe.
    try {
      const gemerkt = (this.homey.settings.get('dp_snapshot') || {})[this.getData().id];
      if (gemerkt?.cids && typeof gemerkt.cids === 'object') this._cidSeen = gemerkt.cids;
    } catch (e) {}

    this._ensureCommandGapSetting(2000);

    this._startAutoReconnect();
  }

  /**
   * Traegt command_gap_ms nach, wo Homey es nicht getan hat.
   *
   * Eine Einstellung, die eine spaetere Version hinzufuegt, bekommt ihre Vorgabe aus
   * dem Manifest nur beim Einrichten eines Geraets. Auf allem, was schon existierte,
   * bleibt das Feld leer und getSetting liefert null - das sieht nicht nur kaputt aus,
   * es hiesse ohne Nachtrag auch, dass ausgerechnet der Bestand weiter ohne Abstand
   * sendet.
   *
   * Zweimal versucht, weil ein Schreibvorgang rund um onInit derjenige ist, den Homey
   * am ehesten fallen laesst: einmal kurz nach dem Init, und noch einmal, wenn das
   * Geraet verbunden ist. Nach dem ersten Erfolg ist der Wert gesetzt und beide Wege
   * kehren sofort zurueck.
   *
   * Faengt alles ab. Dies laeuft in _baseInit, und _baseInit wird von jedem Treiber
   * awaited - ein Wurf hier wuerde die Geraeteinitialisierung mitreissen, und das
   * waere ein hoher Preis fuer einen Vorgabewert.
   *
   * @param {number} delayMs 0 = sofort, sonst verzoegert.
   */
  _ensureCommandGapSetting(delayMs = 0) {
    try {
      if (this._gapBackfillDone) return;
      if (this.getSetting('command_gap_ms') != null) { this._gapBackfillDone = true; return; }
      ...
      const write = () => {
        ...
        this.setSettings({ command_gap_ms: DEFAULT_COMMAND_GAP_MS })
          .then(() => {
            this._gapBackfillDone = true;
            this._appLog(`Minimum gap between commands set to ${DEFAULT_COMMAND_GAP_MS} ms `
              + '(new setting, absent on this device)', 'info');
          })
          .catch((err) => this._appLog(
            `Could not fill in the minimum gap between commands: ${err.message}. `
            + `Commands are spaced by ${DEFAULT_COMMAND_GAP_MS} ms regardless — the field `
            + 'only stays empty until you save a value there yourself.', 'warn'));
      };

      if (!delayMs) { write(); return; }
      if (this._gapBackfillTimer) return;
      this._gapBackfillTimer = this.homey.setTimeout(write, delayMs);
    } catch (err) {
      this._appLog(`Could not fill in the minimum gap between commands: ${err.message}`, 'warn');
    }
  }
  ...
  /**
   * @param {boolean} [keep] Gehoert in den Support-Bericht, auch wenn es nur info ist.
   *   Der Bericht fuehrte bisher nur Warnungen und Fehler - "Connected" fiel damit
   *   heraus, und ein Bericht, der Ausfaelle zeigt, aber nicht was dazwischen war,
   *   laesst die wichtigste Frage offen: kommt das Geraet ueberhaupt herein.
   */
  _appLog(message, level = 'info', keep = false) {
    this.log(message);
    try { this.homey.app.addLog(this.getName(), message, level, keep); } catch (e) {}
  }
  ...
  /**
   * Haelt fest, welche Untergeraete ein Gateway ueber diese Verbindung meldet.
   *
   * Ein Gateway schickt die Werte seiner Zigbee- und Bluetooth-Geraete ueber dieselbe
   * Verbindung wie seine eigenen. Unterschieden sind sie nur durch ein Feld cid im
   * Rohpaket — dieselbe Kennung, die man umgekehrt mitschicken muss, um ein
   * Untergeraet anzusprechen. Bisher ueberschrieb jedes Paket die vorige, und
   * abzulesen war sie nirgends.
   *
   * Dass hier ueberhaupt etwas ankommt, ist die eigentliche Auskunft: es beweist,
   * dass das Gateway seine Untergeraete lokal herausgibt, statt sie nur an die Cloud
   * weiterzureichen. Kommt nichts, ist auch das eine Antwort, und eine schnelle.
   *
   * Die Werte werden je cid getrennt gehalten und nicht unter die eigenen des
   * Gateways gemischt: DP 2 eines Thermostats und DP 2 des Gateways sind zwei
   * verschiedene Dinge, und wer die Tabelle liest, muss das sehen koennen.
   */
  _recordCid(cid, dps) {
    const jetzt = Date.now();
    const e = this._cidSeen[cid] || (this._cidSeen[cid] = {
      dps: {}, pakete: 0, zuerst: jetzt, zuletzt: jetzt,
    });
    e.pakete++;
    e.zuletzt = jetzt;
    for (const [dp, wert] of Object.entries(dps || {})) e.dps[dp] = wert;

    // Ein Gateway mit vielen Untergeraeten darf den Schnappschuss nicht sprengen.
    // Die aeltesten weichen, denn gesucht wird das, was sich gerade gemeldet hat.
    const kennungen = Object.keys(this._cidSeen);
    if (kennungen.length > MAX_CIDS) {
      kennungen
        .sort((a, b) => this._cidSeen[a].zuletzt - this._cidSeen[b].zuletzt)
        .slice(0, kennungen.length - MAX_CIDS)
        .forEach((k) => delete this._cidSeen[k]);
    }

    if (e.pakete === 1) {
      this._appLog(`Sub-device ${cid} reported through this gateway: `
        + `${Object.keys(dps || {}).join(', ') || 'no data points'}. This is the id needed `
        + 'to address it; DP Debug lists it under Sub-devices.', 'info');
    }
    // Der Schnappschuss ist entprellt, ein Aufruf kostet also nichts. Er muss hier
    // stehen: meldet sich nur ein Untergeraet, aendert sich am Gateway selbst nichts,
    // und der Weg, der sonst schreibt, laeuft gar nicht erst an.
    this._writeDpSnapshot();
  }
   ...
   *
   * Beschnitten und laut, beides aus demselben gemeldeten Grund: ein zu grosser Wert
   * wird von Homeys Einstellungen nicht angenommen, und wenn das Scheitern in einem
   * stummen catch verschwindet, merkt es niemand - bis nach einem Neustart nichts mehr
   * da ist. Also wird die Groesse gemessen, bevor geschrieben wird, und ein Fehler
   * kommt ins Protokoll.
   */
  _writeDpSnapshot() {
    ...
    this._snapshotDebounceTimer = setTimeout(() => {
      try {
        ...
        // Der Eintrag wird ersetzt, nicht ergaenzt — fuer die Untergeraete war das
        // zerstoererisch. Steht im Speicher weniger als abgelegt ist, etwa gleich nach
        // einem Neustart, loeschte das erste ankommende Paket den Rest unwiderruflich.
        // Ein Meldender verlor so sein Bluetooth-Thermostat: es stand im Schnappschuss,
        // der Neustart begann mit einer leeren Liste, das Zigbee-Geraet meldete sich
        // zuerst, und damit war der Eintrag ueberschrieben, bevor das langsamere
        // ueberhaupt zu Wort kam. Was einmal gesehen wurde, bleibt jetzt stehen.
        const vereinigt = { ...(snapshot[this.getData().id]?.cids || {}), ...(this._cidSeen || {}) };
        const kennungen = Object.keys(vereinigt);
        if (kennungen.length > MAX_CIDS) {
          kennungen
            .sort((a, b) => (vereinigt[a]?.zuletzt || 0) - (vereinigt[b]?.zuletzt || 0))
            .slice(0, kennungen.length - MAX_CIDS)
            .forEach((k) => delete vereinigt[k]);
        }
        snapshot[this.getData().id] = {
          ...
          // Nur vorhanden, wenn sich ueberhaupt ein Untergeraet gemeldet hat. Bei den
          // allermeisten Geraeten bleibt der Schnappschuss damit so gross wie bisher.
          ...(kennungen.length ? { cids: vereinigt } : {}),
          updatedAt: Date.now(),
        };
        // Der eigene Eintrag bleibt immer, auch wenn er allein die Grenze reisst -
        // sonst schriebe das gespraechigste Geraet sich selbst nie hinein. Geopfert
        // werden die aeltesten der uebrigen, denn der Schnappschuss ist zum Nachsehen
        // da und das Aelteste hilft am wenigsten.
        const eigene = this.getData().id;
        let zuGross = JSON.stringify(snapshot).length > SNAPSHOT_MAX_CHARS;
        if (zuGross) {
          const nachAlter = Object.entries(snapshot)
            .filter(([id]) => id !== eigene)
            .sort((a, b) => (a[1]?.updatedAt || 0) - (b[1]?.updatedAt || 0));
          const gestrichen = [];
          while (zuGross && nachAlter.length > 0) {
            const [id] = nachAlter.shift();
            delete snapshot[id];
            gestrichen.push(id);
            zuGross = JSON.stringify(snapshot).length > SNAPSHOT_MAX_CHARS;
          }
          this._appLog(
            `DP snapshot exceeded ${Math.round(SNAPSHOT_MAX_CHARS / 1024)} KB — dropped `
            + `the oldest ${gestrichen.length} device entr`
            + `${gestrichen.length === 1 ? 'y' : 'ies'} to keep the write within what `
            + 'Homey settings accept. DP Debug shows less history; nothing else is '
            + 'affected.', 'warn');
        }
        this.homey.settings.set('dp_snapshot', snapshot);
      } catch (e) {
        // Nicht mehr verschluckt: genau hier ist es bei einer anderen App unbemerkt
        // schiefgegangen, und der Preis war ein leeres Register nach dem Neustart.
        this._appLog(`Could not save the DP snapshot: ${e.message}`, 'warn');
      }
    }, SNAPSHOT_DEBOUNCE_MS);
  }
  ...
  async _connectInner() {
    ...
    // Bei jedem Verbindungsaufbau neu gelesen, wie der Kommando-Abstand auch: so
    // greift ein Umlegen des Schalters beim naechsten Versuch, ohne dass jeder
    // Treiber ihn durch onSettings faedeln muesste.
    this._conn.setProtocolLocked(this.getSetting('lock_protocol') === true);

    // Die Rotation hat herausgefunden, welche Version das Geraet wirklich spricht.
    // Festhalten, sonst beginnt der naechste Verbindungsaufbau wieder bei der
    // eingestellten und laeuft in denselben Handshake-Ablauf.
    //
    // Nicht angeruehrt, wenn der Nutzer das Protokoll gesperrt hat: dann rotiert ohnehin
    // nichts, und was in der Einstellung steht, steht dort mit Absicht.
    //
    // `version` gehoert in den Treibern zu den Schluesseln, die onSettings neu verbinden
    // laesst. Falls Homey das auch fuer eine programmatische Aenderung tut, ist die Folge
    // ein einziger zusaetzlicher Verbindungsaufbau - mit der jetzt richtigen Version, die
    // sofort traegt. Das ist der Preis dafuer, den Fehler nicht dauerhaft zu wiederholen.
    this._conn.on('version-proven', (version) => {
      if (this.getSetting('lock_protocol') === true) return;
      if (this.getSetting('version') === version) return;
      this.setSettings({ version })
        .then(() => this._appLog(`Protocol Version set to ${version}`, 'info'))
        .catch(() => {});
    });

    this._conn.on('connected', () => {
      this._appLog('Connected', 'info', true);
      ...
      this._lastDps = {};
      // Zweiter Anlauf fuer den Nachtrag. Der erste laeuft kurz nach onInit; wenn
      // Homey ihn dort fallen laesst, ist hier der sichere Zeitpunkt - das Geraet
      // steht vollstaendig. Ist der Wert bereits gesetzt, kehrt die Methode sofort
      // zurueck, das kostet also nichts.
      this._ensureCommandGapSetting(0);
    ...
    // Ein Versuch, der nie zustande kam.
    //
    // Gemeldet von Jean-Claude Jordens: fuenf ausgesteckte Steckdosen, im Protokoll
    // seit dreizehn Versuchen EHOSTUNREACH, und Homey zeigte sie als verfuegbar. Die
    // App wusste es — "Device not connected" kam ja beim Schalten —, nur sagte es
    // niemand weiter. Unerreichbar wurde bisher allein aus dem Abbruch einer
    // bestehenden Verbindung geschlossen, und ein Geraet, das seit dem App-Start nie
    // verbunden war, hat nie eine abgebrochen. Es blieb auf Homeys Vorgabe stehen.
    //
    // Dass es "zufaellig" aussah, passt dazu: es hing daran, ob das Geraet in diesem
    // App-Lauf einmal erreichbar gewesen war. Ein Neustart machte aus unerreichbar
    // wieder verfuegbar, ohne dass sich am Geraet irgendetwas geaendert haette.
    //
    // Dieselbe Schonfrist wie beim Abbruch: ein Geraet, das beim App-Start noch
    // hochfaehrt, soll nicht sofort als tot dastehen.
    this._conn.on('connect-failed', (reason) => {
      if (this._unavailableTimer) return;          // laeuft schon
      if (this._conn?.connected)   return;
      const gnade = (this.getSetting('offline_grace_seconds') ?? 60) * 1000;
      this._unavailableTimer = setTimeout(() => {
        this._unavailableTimer = null;
        if (this._conn?.connected) return;
        this.setUnavailable(reason || 'Device unreachable').catch(() => {});
    ...
    this._conn.on('data', (dps, raw) => {
      ...
      // Ein Paket mit fremder cid gehoert einem Untergeraet hinter diesem Gateway und
      // nicht dem Gateway selbst. Es hier abzubiegen ist kein Feinschliff: ein
      // gemeldetes Gateway fuehrte DP 2, 16, 19, 20 und 24 in seiner eigenen Liste,
      // obwohl alle fuenf von seinen zwei Thermostaten stammten — und weil beide ein
      // DP 2 haben, ueberschrieben sie einander darin auch noch. Ein Treiber, der auf
      // DP 2 eine Faehigkeit gelegt hat, haette dem Gateway den Zustand eines
      // Thermostats angezeigt.
      //
      // Die eigene Kennung ist ausgenommen: schickt ein Geraet seine eigene cid mit,
      // sind es seine eigenen Werte.
      const fremd = raw?.cid && String(raw.cid) !== String(this.getSetting('device_id') ?? '');
      if (fremd) {
        this._recordCid(String(raw.cid), dps);
        return;
      }
    ...
    this._conn.on('log', ({ message, level, keep }) => this._appLog(message, level, keep));
  ...
  /**
   * Beruehrt diese Einstellungsaenderung ein Feld, an dem eine optionale Faehigkeit
   * haengt?
   *
   * Die Methode gibt es wegen eines Fehlers, den die naheliegende Schreibweise hat:
   *
   *     changedKeys.some((k) => optionals.map((o) => o.setting).includes(k))
   *
   * Das findet nichts, sobald `setting` ein Array ist — und Array ist es ueberall
   * dort, wo zwei Datenpunkte dieselbe Faehigkeit rechtfertigen: Spannung und Strom
   * am Ladegeraet, der Bewegungs-DP der Tuerklingel, die eigenen Schwellen des
   * Fuellstandsensors. `.includes('dp_phase_json')` gegen ein Array von Arrays ist
   * immer falsch, ohne dass es auffaellt.
   *
   * Die Folge war leise: wer ein solches Feld setzte, sah seine Kachel nicht — erst
   * ein Neustart der App holte sie nach. Gemeldet hat es der erste Benutzer eines
   * Ladegeraets mit JSON-Phasenblock, dessen Sitzungsenergie darum nie ankam.
   *
   * @param {string[]} changedKeys
   * @param {Array<{setting: string|string[]}>} optionals
   * @returns {boolean}
   */
  _touchesOptional(changedKeys, optionals = []) {
    const felder = new Set(optionals.flatMap((o) => [].concat(o.setting)));
    return (changedKeys || []).some((k) => felder.has(k));
  }
   ...
   */
  async _syncEnumOptions(capabilityId, csv, { zuordnung = false } = {}) {
    if (!this.hasCapability(capabilityId)) return;
    const capitalize = (v) => v.charAt(0).toUpperCase() + v.slice(1).replace(/_/g, ' ');

    // "cool=1" heisst: zeige cool, sende 1. Nur Treiber, die die Uebersetzung auch
    // beim Lesen und Senden vornehmen, duerfen das anbieten — sonst stuenden im
    // Waehler huebsche Namen, und gesendet wuerde der Name statt der Zahl.
    if (!zuordnung && hatZuordnung(csv)) {
      this._appLog(`${capabilityId}: the name=value form is not supported here yet — this `
        + 'driver sends the values as written. Enter the values your device itself uses.', 'warn');
      return;
    }

    const opts = leseWerteliste(csv)
      .map((e) => ({ id: e.id, title: { en: capitalize(e.id), de: capitalize(e.id) } }));
    if (opts.length === 0) return;

    let currentValue = this.getCapabilityValue(capabilityId);

    // Der Wechsel von blossen Werten zu einer Zuordnung scheiterte an der Pruefung
    // darunter, und zwar bei jedem, der ihn vornahm.
    //
    // Wer vorher "0,1,2,3,4" eingetragen hatte, stand mit einem rohen "4" in der
    // Faehigkeit. Traegt er nun "auto=0,…,heat=4" ein, ist "4" in der neuen Liste
    // nicht mehr enthalten — die Liste heisst jetzt auto bis heat —, also wurde die
    // Umstellung abgelehnt und die alte Auswahl blieb stehen. Im Waehler standen
    // weiter Ziffern, und ankommende Werte wurden als "heat" gesetzt, was die alte
    // Auswahl nicht kennt: verworfen. Beide Beschwerden, eine Ursache.
    //
    // Ist der aktuelle Wert also ein Geraetewert, der sich zuordnen laesst, wird er
    // mit umgestellt, statt die Umstellung zu verweigern.
    if (zuordnung && currentValue !== null && currentValue !== undefined
        && !opts.some((o) => o.id === currentValue)) {
      const uebersetzt = rohZuId(csv, currentValue);
      if (opts.some((o) => o.id === uebersetzt)) {
        await this.setCapabilityValue(capabilityId, uebersetzt).catch(() => {});
        this._appLog(`${capabilityId}: "${currentValue}" is the device's own value for `
          + `"${uebersetzt}" — carried over so the new list can take effect.`, 'info');
        currentValue = uebersetzt;
      }
    }
  ...
  /**
   * Flow-Ausloeser fuer Capabilities, deren Wechsel gemeldet werden soll.
   *
   * Homey erzeugt "wurde eingeschaltet" nur fuer seine eigenen Capabilities. Fuer
   * child_lock, oscillate und die uebrigen von der App definierten entsteht nichts,
   * also muss der Treiber selbst merken, dass sich etwas geaendert hat.
   *
   * @param {Object<string,string>} map  Capability -> Kartenbezeichner
   */
  _registerChangeTriggers(map) {
    this._changeTriggers = {};
    for (const [capability, cardId] of Object.entries(map)) {
      this._changeTriggers[capability] = this.homey.flow.getDeviceTriggerCard(cardId);
    }
  }

  /**
   * Schreibt eine Capability und meldet einen echten Wechsel an ihren Ausloeser.
   *
   * Ohne registrierten Ausloeser genau das, was vorher an dieser Stelle stand - die
   * Treiber koennen also alle darauf umgestellt werden, auch die ohne Ausloeser.
   *
   * Der erste Wert nach dem Verbinden loest nicht aus. Vorher steht dort null, und das
   * ist kein Wechsel, sondern der erste Blick auf einen Zustand, den das Geraet die
   * ganze Zeit schon hatte. Sonst feuerte jede Reconnect-Welle eine Runde Flows.
   */
  /**
   * Eine feste Wattzahl melden, statt Homey schaetzen zu lassen.
   *
   * Homey multipliziert den hinterlegten Verbrauch mit dem Dimmwert, sobald ein Geraet
   * `dim` fuehrt. Bei vielen LED-Leuchten ist das schlicht falsch: sie ziehen bei jeder
   * Dimmstufe nahezu dasselbe, und wer viele davon hat, verzerrt damit seine ganze
   * Energiebilanz. Ein *gemessenes* measure_power hat bei Homey Vorrang vor jeder
   * Schaetzung - also liefert die App eines, sobald jemand die Wattzahl kennt.
   *
   * Untaetig, solange power_on_watts fehlt oder 0 ist. Treiber ohne diese Einstellung
   * kommen hier durch, lesen undefined und kehren sofort zurueck; die Faehigkeit
   * entsteht bei 0 gar nicht erst.
   *
   * @param {boolean} [an] Der Zustand, falls er noch nicht in der Faehigkeit steht -
   *   im Capability-Listener laeuft dieser Aufruf, bevor Homey den Wert uebernimmt.
   */
  async _applyFixedPower(an) {
    const ein = Number(this.getSetting('power_on_watts'));
    if (!(ein > 0)) return;
    if (!this.hasCapability('measure_power')) return;
    const zustand = an === undefined ? this.getCapabilityValue('onoff') === true : an === true;
    const aus  = Number(this.getSetting('power_off_watts'));
    const watt = zustand ? ein : (Number.isFinite(aus) && aus > 0 ? aus : 0);
    await this.setCapabilityValue('measure_power', watt).catch(() => {});
  }

  async _applyCapability(capability, value) {
    // Umschlossen statt umgeschrieben: die Ausloeser-Logik darunter bleibt Wort fuer
    // Wort, wie sie war, und die Leistung haengt sich hinten an. Sie an den drei
    // Ausgaengen der Methode einzeln nachzutragen hiesse, drei Stellen richtig zu
    // halten statt einer.
    await this._applyCapabilityWert(capability, value);
    if (capability === 'onoff') await this._applyFixedPower(Boolean(value));
  }

  async _applyCapabilityWert(capability, value) {
    if (!this.hasCapability(capability)) return;
    const card = this._changeTriggers?.[capability];
    if (!card) {
    ...
    await this.setCapabilityValue(capability, value).catch(() => {});
    if (prev === null || prev === undefined || prev === value) return;
    // Der Wert reist im Zustand, nicht als Token: die Karte traegt eine Auswahl
    // "eingeschaltet / ausgeschaltet", und der Run-Listener vergleicht beides.
    card.trigger(this, {}, { enabled: String(value) }).catch(() => {});
  }
