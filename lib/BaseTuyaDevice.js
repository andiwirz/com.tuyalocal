...
class BaseTuyaDevice extends Homey.Device {
   ...
   *
   * Deliberately narrow, the same rule as at pairing: only a numeric declared list
   * against a prefix_number token, which is plainly the same thing written two
   * ways. The count stays as declared, the spelling comes from the device.
  ...
  async _reconcileEnumToken(capabilityId, settingKey, token) {
    ...
    if (!m || !m[1] || !declared.every((v) => /^\d+$/.test(v))) {
    ...
    const rebuilt = declared.map((n) => `${m[1]}${n}`).join(',');
