...
class TuyaConnection extends EventEmitter {
  ...
  async connect() {
    ...
    this._tuya.on('error', (err) => {
      ...
      const hint    = isReset
        ? ' — likely protocol version mismatch. Use Auto-detect in the device Repair screen.'
        : '';
      this._emit('log', `Error: ${msg}${hint}`, 'error');
    ...
    try {
      ...
      const hint    = isReset
        ? ' — likely protocol version mismatch. Use Auto-detect in the device Repair screen.'
        : '';
      this._emit('log', `Connection failed: ${msg}${hint}`, 'error');
