...
class TuyaConnection extends EventEmitter {
  ...
  /** True only after the current protocol produced an application-level response. */
  get protocolConfirmed() { return this._connectionConfirmed; }
  ...
  _scheduleReconnect(reason = '') {
    ...
    // Network-unreachable failures neither advance nor trigger protocol rotation:
    // no protocol can reach a device that is not present at its configured address.
    const isProtocolFailure = !this._connectionConfirmed && !isUnreachable(reason);
    if (isProtocolFailure) this._protocolFailures++;

    // After VERSION_ROTATE_AFTER consecutive unconfirmed protocol failures, try
    // the next version. This also handles firmware that changed protocol after OTA.
    if (isProtocolFailure && this._protocolFailures % VERSION_ROTATE_AFTER === 0) {
