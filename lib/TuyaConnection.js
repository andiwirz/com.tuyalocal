...
// Nach wie vielen Fehlschlaegen in Folge auch eine Version wieder in Frage gestellt
// wird, unter der schon Daten ankamen. Firmware kann per OTA die Version wechseln, und
// ein Geraet, das danach nie wieder gefunden wird, waere schlimmer als eine
// ueberfluessige Suche. Hoch genug, dass ein gemeldeter Fall - zwanzig Fehlschlaege in
// vier Minuten an einem Geraet, das kurz zuvor 903 Pakete geliefert hatte - die Suche
// nicht mehr ausloest.
const PATIENCE_AFTER_DATA = 60;
...
class TuyaConnection extends EventEmitter {
  constructor({ id, key, ip, version }) {
    ...
    // Die Version, unter der zuletzt etwas Entziffertes ankam. Der staerkste Beleg, den
    // es gibt: hier hat nicht nur die Rahmung gestimmt, sondern auch der Schluessel.
    // _versionProven faellt bei jedem Verbindungsaufbau zurueck - zu Recht, eine offene
    // Leitung beweist nichts -, aber empfangene Daten sind kein Zustand der Leitung,
    // sondern eine Tatsache ueber das Geraet, und die bleibt.
    this._versionThatWorked = null;
  ...
  async connect() {
    ...
    this._tuya.on('error', (err) => {
      ...
      //
      // Der HMAC-Fehler faellt hier zweimal aus der Reihe.
      //
      // Einmal beim Zusammenfassen: kuerzeHex laesst von jeder Signatur die ersten
      // sechzehn Zeichen stehen, also ist jede Zeile einmalig und _throttleLog - das
      // Zeilen vergleicht - kann nie greifen. Genau daran ist der geduldete Zweig
      // daneben schon einmal gescheitert; geheilt wurde damals nur er. In einem
      // gemeldeten Bericht standen elf dieser Zeilen, acht davon in zwei Minuten.
      // Ohne den Schluessel sagt die Signatur ohnehin niemandem etwas.
      //
      // Und einmal beim Hinweis: _failureHint kennt jeden anderen haeufigen Fehler,
      // nur diesen nicht. Er braucht aber die Verbindung, um beantwortet zu werden,
      // und ist deshalb kein statischer Zweig, sondern _hmacHint().
      const hint = this._isIdleReset(msg) ? this._idleResetHint()
        : hmac ? this._hmacHint()
        : this._exhaustedResetHint(msg) || TuyaConnection._failureHint(msg);
      const text = hmac ? fehlerart(msg) : kuerzeHex(msg);
      this._throttleLog(`${text}${hint}`, isTimeout(msg) ? 'warn' : 'error');
    ...
    try {
      ...
      const hint = this._exhaustedResetHint(msg) || TuyaConnection._failureHint(msg);
      ...
      // Ein gescheiterter Versuch ist kein Abbruch: _handleDisconnect kehrt bei
      // !_connected sofort zurueck, und das ist richtig so. Nur hat damit niemand
      // erfahren, dass ein Geraet unerreichbar ist, das noch nie erreichbar war —
      // nach einem App-Start blieb ein seit Wochen ausgestecktes Geraet auf Homeys
      // Vorgabe stehen, und die heisst "verfuegbar".
      this.emit('connect-failed', msg);
  ...
  _noteGetUnanswered() {
    ...
    this._throttleLog(
      ...
      + 're-added in the Smart Life app. With cloud credentials saved, Settings → Fix It '
      + '→ Check local keys makes that comparison for every device in one press.',
      'warn');
  }
  ...
  _noteDeviceAnswered(solicited = false) {
    ...
    this._versionThatWorked = this._version;
  ...
  static _failureHint(msg) {
    ...
    // Der Fall "alle Versionen schon durch" steht nicht hier, sondern in
    // _exhaustedResetHint: er braucht die Verbindung, und dies ist statisch.
  ...
  /**
   * ECONNRESET, nachdem die Rotation alle Versionen ohne eine Antwort durchhatte.
   *
   * Die App stellt an dieser Stelle selbst fest, die Version sei nicht die Ursache —
   * und riet dreizehn Sekunden spaeter wieder, sie von Hand zu setzen. Ein Meldender
   * hat daraufhin 3.3, 3.4 und 3.5 durchprobiert, was nichts aendern konnte. Zwei
   * richtige Saetze, die einander aufheben, sind schlechter als einer.
   *
   * Instanzmethode, nicht statisch: die Antwort haengt am Zustand der Verbindung,
   * genau wie bei _hmacHint und _idleResetHint.
   *
   * @returns {string} leer, solange der Fall nicht vorliegt
   */
  _exhaustedResetHint(msg) {
    if (!msg.includes('ECONNRESET')) return '';
    if (!this._versionsExhausted)    return '';
    return ' — every protocol version has already been tried here without a single reply,'
      + ' so this is not a version problem. A device that resets the connection this way'
      + ' normally has its one local connection slot taken: these accept exactly one.'
      + ' Close the Tuya or Smart Life app, check for another integration, and make sure'
      + ' no second device in this app points at the same address.';
  }
  ...
  /**
   * Ein HMAC-Fehler - die Antwort haengt davon ab, wie weit die Versionssuche ist.
   *
   * Die Signatur wird aus dem lokalen Schluessel gerechnet und sonst aus nichts. Was
   * ankam, ist also ein wohlgeformter Rahmen; wir koennen nur seine Unterschrift nicht
   * nachbilden. Dafuer gibt es zwei Ursachen, und welche zutrifft, weiss die Verbindung
   * selbst:
   *
   *   Die Rotation ist noch unterwegs - dann kann es genauso die falsche Rahmung sein.
   *     Was wir als Signatur lesen, sind bei 3.3 schlicht andere Bytes. Weitersuchen.
   *
   *   Alle Versionen einmal durch, nie ein lesbares Paket - dann faellt die Rahmung als
   *     Erklaerung weg und der Schluessel bleibt als einziger uebrig.
   *
   * @returns {string}
   */
  _hmacHint() {
    if (!this._versionsExhausted) {
      return ' — the signature on that frame does not match the one computed from the'
        + ` Local Key. On ${this._version} that is either a wrong key or the wrong framing`
        + ' altogether, and those look alike from here, so the app finishes trying the'
        + ' other versions before drawing a conclusion.';
    }
    return ' — the signature on that frame is computed from the Local Key and nothing'
      + ' else. Every protocol version has now been tried without a single readable'
      + ' packet, so the framing is not the cause and the stored Local Key does not'
      + ' belong to this device. It changes every time the device is re-added in the'
      + ' Smart Life app. With Tuya cloud credentials saved, Settings → Fix It →'
      + ' Check local keys compares every device against your account and corrects the'
  ...
  _scheduleReconnect(reason = '') {
    ...
    // Steht die laufende Version fest - aus den Rahmen gelesen oder durch empfangene
    // Daten belegt -, wird nicht gesucht. Bei belegten Daten mit einem Hintertuerchen:
    // nach PATIENCE_AFTER_DATA Fehlschlaegen doch, falls eine Firmware die Version
    // gewechselt hat.
    const stehtFest = (this._versionFromFrames && this._version === this._versionFromFrames)
      || (this._versionThatWorked && this._version === this._versionThatWorked
          && this._reconnectAttempts < PATIENCE_AFTER_DATA);
    if (stehtFest) {
