...
// Ceiling fittings that are a light with a fan in them, rather than a fan with a
// lamp bolted on: the Tuya "fsd"/"xdd" combo families and the 52 equivalents
// catalogued by the tuya-local project.
//
// Standalone by request. It shares its mechanics with the fan driver — the same
// packed-HSV colour handling, the same debounced sliders, the same fake-state — but
// the two are separate files and a fix to one does not reach the other. Anything
// repaired here that also applies to drivers/fan is worth carrying across by hand.
//
// What differs from the fan driver, and why this is not just a class change: Homey
// files a device under one class and drives it through one `onoff`. On a driver
// declared as a light both have to belong to the light, or "turn off all the lights
// on this floor" turns off the fan and leaves the lamp burning.
//
//   drivers/fan          drivers/fan_light
//   ───────────          ─────────────────
//   onoff       = fan    onoff      = light
//   dim         = speed  dim        = brightness
//   onoff.light = light  onoff.fan  = fan
...
// runs 0–1000 and 0 means off, while dim legitimately reaches 0 — at the
...
const DP_PROFILE = [
  { settingKey: 'dp_light_onoff',     capability: 'onoff',           transform: (v) => Boolean(v),  settable: true  },
  { settingKey: 'dp_onoff',           capability: 'onoff.fan',       transform: (v) => Boolean(v),  settable: true  },
...
const OPTIONAL_CAPABILITIES = [
  ...
  // The light half. onoff and dim are declared in the manifest because a fitting
  // without a lamp does not belong on this driver at all.
...
class FanLightDevice extends BaseTuyaDevice {
  async onInit() {
    ...
    // ── Flow trigger cards ──────────────────────────────────────────────────
    this._triggerDeviceConnected    = this.homey.flow.getDeviceTriggerCard('fanlight_device_connected');
    this._triggerDeviceDisconnected = this.homey.flow.getDeviceTriggerCard('fanlight_device_disconnected');
    ...
    this._registerChangeTriggers({
      child_lock: 'fanlight_child_lock_changed',
      oscillate: 'fanlight_oscillate_changed',
    });
  ...
  _registerListeners() {
    ...
    // ── dim.fan (fan speed 0–1) ─────────────────────────────────────────────
    register('dim.fan', (value) => {
    ...
    for (const [dpStr, value] of Object.entries(dps)) {
      ...
      if (settings.dp_speed > 0 && dp === settings.dp_speed) {
        ...
        const dim = speedMax > speedMin
          ...
          : 0;
        await this.setCapabilityValue('dim.fan', dim).catch(() => {});
        continue;
      }
      ...
      if (settings.dp_light_dim > 0 && dp === settings.dp_light_dim) {
        if (this.hasCapability('dim')) {
          ...
          const dimVal = max > min
            ...
            : 0;
          await this.setCapabilityValue('dim', dimVal).catch(() => {});
        }
        continue;
      }
      ...
      if (settings.dp_light_colour > 0 && dp === settings.dp_light_colour) {
        ...
        if (parsed) {
          ...
          if (this.hasCapability('dim') && this.getCapabilityValue('light_mode') === 'color') {
