...
**Version 1.0.232** · Local WiFi/LAN control of Tuya smart devices — no cloud, no Zigbee hub required.
...
| A multi-gang switch shows only one channel | It was paired on the **Smart Plug** driver, which has a single `dp_switch`. Its cloud specification gives it away: `switch_1` … `switch_4` and no power, voltage or current at all | Remove it and pair it on the **Wall Switch** driver, which has `dp_switch_1` … `dp_switch_4`. A device cannot change driver. Countdowns are not auto-detected — enter `dp_countdown_1` … `_4` by hand if you want per-channel timers |
...
| Device connects and stays connected, but `data ever received: NO` | The address may belong to a **different** Tuya device. One that is not yours still accepts the connection and answers keep-alives — those need no key — and discards everything encrypted, which looks exactly like a wrong key | Start **Add device → Tuya Local → (any driver)** and press **Scan Network**, then cancel. Tuya devices announce their Device ID in the clear, so the list shows which ID is at which address. Check this **before** chasing the Local Key, and give the device a DHCP reservation afterwards |
| Log repeats `HMAC mismatch` and no values ever arrive | On 3.4/3.5 the signature on every frame is computed from the Local Key and nothing else, so a mismatch means the stored key does not belong to this device. It changes every time the device is re-added in the Smart Life app | **Fix It** → **Check local keys** compares every device against your Tuya account and corrects the ones that no longer match. Without cloud credentials, re-copy the key from the Tuya IoT Platform |
| **Check local keys** reports an error and compares nothing | Your Tuya project refuses to *list* its devices. Listing and per-device access are permissioned separately, so the specification of a device can be readable while the account listing is not | The check now falls back to asking for each device by its id. If it still finds nothing, open the Tuya IoT Platform → your project → **Link Tuya App Account** and re-link your Smart Life account. The support report names the exact refusal |
| EV charger reports voltage and current per phase, but the fields stay empty | Some chargers name those data points themselves (`A_Voltage`, `A_Current`, `DeviceKw`) instead of using the standard code names | Cloud Lookup now recognises those names — pair the device again, or read the numbers off **DP Debug** and enter `dp_voltage_a` … `dp_current_c` by hand |
| EV charger: Session Energy stays empty and the total never moves | On chargers with vendor code names, DP 25 (`charge_energy_once`) is never sent — their session counter is **DP 112** (`DeviceKwh`), which rises during a charge and returns to 0 afterwards | Set `dp_session_energy = 112`, `dp_energy_total = 0` and **Total Energy Source** to *session*. The app then builds the lifetime total from completed sessions. Watch the Logs tab: it names any unmapped data point that is counting up |
| EV charger: voltage reads ~2400 V, power is far too low | Those chargers scale differently on all three quantities, and `DeviceKw` counts **tenths of a kilowatt**, not watts | `voltage_scale` **0.1**, `amp_scale` **0.1**, `power_scale` **100**. Check the result against itself: volts × amps × phases must come out near the reported power |
| EV charger: Charging state stays empty | The charger reports its state on DP 101 (`DeviceState`: `no_connet`, `connect`, `charing`, …) rather than on DP 3 (`work_state`), which it never sends | Not yet read by the app. Set `dp_work_state = 0` so the setting does not look configured; the tile fills once support for that vocabulary ships |
